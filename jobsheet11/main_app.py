...
def halaman_riwayat(anggaran: AnggaranHarian):
    ...
    with st.expander("🗑️ Hapus Transaksi berdasarkan ID"):
        id_hapus = st.number_input("ID Transaksi yang ingin dihapus:", min_value=1, step=1, key="input_id_hapus")

        if st.button("Hapus Transaksi Terpilih", key="btn_hapus"):
            st.session_state.konfirmasi_hapus = id_hapus  # Simpan ID yang ingin dihapus

        if "konfirmasi_hapus" in st.session_state:
            st.warning(f"Anda akan menghapus transaksi dengan ID: {st.session_state.konfirmasi_hapus}. Konfirmasi?", icon="⚠️")
            if st.button("✅ Konfirmasi Hapus", key="btn_konfirmasi_hapus"):
                with st.spinner("Menghapus transaksi..."):
                    if anggaran.hapus_transaksi(st.session_state.konfirmasi_hapus):
                        st.success(f"Transaksi ID {st.session_state.konfirmasi_hapus} berhasil dihapus.", icon="✅")
                        del st.session_state.konfirmasi_hapus
                        st.cache_data.clear()
                        st.rerun()
                    else:
                        st.error(f"Gagal menghapus transaksi ID {st.session_state.konfirmasi_hapus}.", icon="❌")
                        del st.session_state.konfirmasi_hapus
